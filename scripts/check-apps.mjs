// check-apps.mjs — lista zainstalowanych aplikacji w sklepie (read-only)
...
let cursor=null;
for(;;){
  const res=await gql(`query($c:String){ appInstallations(first:50, after:$c){ nodes{ app{ title developerName } } pageInfo{ hasNextPage endCursor } } }`,{c:cursor});
  if(!res.data?.appInstallations){ console.log('FAIL:',JSON.stringify(res).slice(0,200)); break; }
  res.data.appInstallations.nodes.forEach(n=>console.log('-',n.app.title,'| dev:',n.app.developerName));
