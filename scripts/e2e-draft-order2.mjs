// e2e-verify-final.mjs — finalna weryfikacja E2E: kupna złożyłem na GENESIS (R14W).
// Aby przetestować licznik Sold (std), kupuję TERAZ komercyjną: MARS-PLOT-000012 (R01).
...
const prod=await gql(`query{ products(first:1, query:"handle:mars-plot-000012"){ nodes{ variants(first:1){ nodes{ id sku price } } } } }`);
const v=prod.data.products.nodes[0]?.variants.nodes[0];
console.log('variant:',v?.sku,'€'+v?.price);
const d=await gql(`mutation($input:DraftOrderInput!){ draftOrderCreate(input:$input){ draftOrder{ id name } userErrors{ field message } } }`,{input:{lineItems:[{variantId:v.id,quantity:1}],email:'e2e-test@gmail.com',tags:['e2e-test'],note:'E2E sold-counter test 04.09'}});
