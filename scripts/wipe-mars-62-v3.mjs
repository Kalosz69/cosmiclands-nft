// wipe-mars-62-v3.mjs — pełna kasacja Marsa z POPRAWIONĄ składnią productDelete(input:{id})
// Raport crash-safe; pomija już obsłużone; retry z backoffem do 6.
...
console.log('DO USUNIĘCIA:',mars.length,'| JUŻ OK:',report.ok.length,'| FAIL:',report.failed.length);
for(const p of mars){
  if(report.ok.includes(p.handle)||report.failed.some(f=>f.handle===p.handle)) continue;
  try{
